...
class Web3PaymentAdapter:
    ...
    async def verify_funding_tx(self, network: str, tx_hash: str) -> float:
        ...
        if network not in NETWORKS:
            raise ValueError(
                f"TEMPUS_INVALID_NETWORK: Supported networks are {list(NETWORKS.keys())}"
            )
        ...
        try:
            ...
            raise ValueError(
                f"TEMPUS_TX_VERIFICATION_FAILED: Could not fetch transaction. {e!s}"
            )
        ...
        if not tx["to"] or tx["to"].lower() != self.treasury_address.lower():
            raise ValueError(
                "TEMPUS_TX_INVALID_RECIPIENT: Transaction was not sent to the Tempus Treasury Wallet."
            )
