...
def generate_license_key(prefix="tmb_live_", length=32):
    """Generates a secure random license key."""
    # Ensure minimum length of 24 as required by the Gatekeeper
    random_length = max(length - len(prefix), 24 - len(prefix))
    
    alphabet = string.ascii_letters + string.digits
    random_part = ''.join(secrets.choice(alphabet) for i in range(random_length))
...
def main():
    parser = argparse.ArgumentParser(description="Tempus DDB - Generador de Licencias B2B (La Caja Registradora)")
    parser.add_argument("client_name", help="Nombre del cliente B2B")
    parser.add_argument("--tier", choices=["startup", "enterprise", "unlimited"], default="startup", help="Nivel del servicio contratado")
