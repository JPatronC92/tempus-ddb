...
def test_tamper_delete_middle_record(tmp_path):
    ...
    with pytest.raises(Exception) as exc:
        db.validate()
    assert "invalid" in str(exc.value).lower() or "error" in str(exc.value).lower() or "mismatch" in str(exc.value).lower() or "parent" in str(exc.value).lower()
...
def test_tamper_modify_payload(tmp_path):
    ...
    conn = sqlite3.connect(str(db_path))
    try:
        conn.execute("UPDATE decisions SET payload = ?", (json.dumps({"amount": 1000}),))
        conn.commit()
    finally:
        conn.close()
    ...
    with pytest.raises(Exception) as exc:
        db.validate()
    assert "invalid" in str(exc.value).lower() or "error" in str(exc.value).lower() or "mismatch" in str(exc.value).lower()
